...
counters.forEach(counter => {
    const updateCounter = () => {
        ...
        if (count < target) {
            // Increment counter value and update the DOM
            counter.innerText = Math.ceil(count + increment);
            setTimeout(updateCounter, 10); // Call the function repeatedly
        } else {
            counter.innerText = target; // Ensure it doesn't go past the target
        }
    };

    // Trigger counter increment when page is loaded
    updateCounter();
});
...
window.FontAwesomeKitConfig = {
  ...
  // Optional JavaScript for on-scroll animations
